...
export const obtieneDatosSesion = async (id_usuario: number) => {
    try {
        ...
        if (!usuario.length) return { error: 'No se encuentra el usuario' }
        ...
        if (u.id_rol === 2) {
            const [doctor]: any = await conexion.query(
                'SELECT * FROM doctor WHERE id_usuario = ? LIMIT 1', [id_usuario]
            )
            const [sucursales]: any = await conexion.query(
                `SELECT ds.id_sucursal FROM doctor_sucursal ds WHERE ds.id_doctor = ?`,
                [doctor[0]?.id_doctor]
            )
            Object.assign(u, doctor[0] ?? {})
            u.sucursales = sucursales.map((s: any) => s.id_sucursal)
        } else if (u.id_rol === 3) {
            const [recep]: any = await conexion.query(
                'SELECT * FROM recepcionista WHERE id_usuario = ? LIMIT 1', [id_usuario]
            )
            Object.assign(u, recep[0] ?? {})
        } else if (u.id_rol === 4) {
            const [paciente]: any = await conexion.query(
                'SELECT * FROM paciente WHERE id_usuario = ? LIMIT 1', [id_usuario]
            )
