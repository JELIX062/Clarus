...
export const obtieneRecepcionista = async (id_recepcionista: number) => {
    try {
        const [results] = await conexion.query(`
            SELECT r.*, u.nombre, u.apellido_paterno, u.apellido_materno, u.correo, u.telefono
            FROM recepcionista r
...
export const registraRecepcionista = async (nuevo: RecepcionistaNuevo) => {
    try {
        ...
        const [result]: any = await conexion.query(
            ...
            [3, nuevo.nombre, nuevo.apellido_paterno, nuevo.apellido_materno ?? null, nuevo.correo, nuevo.telefono ?? null, hash]
        );
        ...
        const [recepcionista]: any = await conexion.query(
            ...
            [id_usuario, nuevo.id_sucursal, nuevo.turno]
        );
...
export const editaRecepcionista = async (datos: RecepcionistaEditar) => {
    try {
        ...
        await conexion.query(
            ...
            [datos.id_sucursal, datos.turno, datos.id_recepcionista]
        );
...
export const borrarRecepcionista = async (id_recepcionista: number) => {
    try {
        const [recepcionista]: any = await conexion.query(
            'SELECT id_usuario FROM recepcionista WHERE id_recepcionista = ? LIMIT 1',
            [id_recepcionista]
        );
