...
export const obtieneAdministrador = async (id_administrador: number) => {
    try {
        const [results] = await conexion.query(`
            SELECT a.*, u.nombre, u.apellido_paterno, u.apellido_materno, u.correo, u.telefono
            FROM administrador a
...
export const registraAdministrador = async (nuevo: AdministradorNuevo) => {
    try {
        ...
        const [result]: any = await conexion.query(
            ...
            [1, nuevo.nombre, nuevo.apellido_paterno, nuevo.apellido_materno ?? null, nuevo.correo, nuevo.telefono ?? null, hash]
        );
...
export const editaAdministrador = async (datos: AdministradorEditar) => {
    try {
        ...
        let queryUsuario = `UPDATE usuario SET
                ...
                correo = ?,
                telefono = ?`;
        let paramsUsuario: any[] = [datos.nombre, datos.apellido_paterno, datos.apellido_materno, datos.correo, datos.telefono];
        ...
        await conexion.query(
            'UPDATE administrador SET cargo = ? WHERE id_administrador = ?',
            [datos.cargo, datos.id_administrador]
        );
...
export const borrarAdministrador = async (id_administrador: number) => {
    try {
        const [admin]: any = await conexion.query(
            'SELECT id_usuario FROM administrador WHERE id_administrador = ? LIMIT 1',
            [id_administrador]
        );
