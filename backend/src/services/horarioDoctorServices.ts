...
export const registraHorario = async (nuevo: HorarioDoctorNuevo) => {
    try {
        ...
        const [conflicto]: any = await conexion.query(`
        ...
        const [conflictoDoctor]: any = await conexion.query(`
            SELECT id_horario FROM horariodoctor
            WHERE id_doctor    = ?
                AND dia_semana   = ?
                AND activo       = 1
    ...
    try {
        ...
        await conexion.query(
            `UPDATE horariodoctor SET
                ...
                activo = ?
            WHERE id_horario = ?`,
            [datos.id_consultorio, datos.dia_semana, datos.hora_inicio, datos.hora_fin, datos.activo, datos.id_horario]
        );
...
export const borrarHorario = async (id_horario: number) => {
    try {
        const [existe]: any = await conexion.query(
            'SELECT id_horario FROM horariodoctor WHERE id_horario = ? LIMIT 1',
            [id_horario]
