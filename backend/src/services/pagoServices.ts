...
export const obtienePago = async (id_pago: number) => {
    try {
        const [results] = await conexion.query(`
            SELECT pg.*,
                up.nombre as nombre_paciente, up.apellido_paterno as apellido_paciente,
...
export const obtienePagosPorCita = async (id_cita: number) => {
    try {
        const [results] = await conexion.query(`
            SELECT pg.*
            FROM pago pg
...
export const registraPagoFinal = async (nuevo: PagoNuevo) => {
    try {
        ...
        const [anticipo]: any = await conexion.query(
            "SELECT monto FROM pago WHERE id_cita = ? AND tipo_pago = 'Anticipo' AND estado = 'Completado'",
            [nuevo.id_cita]
        );
        ...
        const [pagoFinal]: any = await conexion.query(
            "SELECT id_pago FROM pago WHERE id_cita = ? AND tipo_pago = 'Total'",
            [nuevo.id_cita]
        );
