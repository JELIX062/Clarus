...
export const obtieneDoctores = async () => {
    try {
        const [results]: any = await conexion.query(`
        ...
        for (const doc of results) {
            const [suc]: any = await conexion.query(
                'SELECT id_sucursal FROM doctor_sucursal WHERE id_doctor = ?',
                [doc.id_doctor]
            );
            doc.sucursales = suc.map((s: any) => s.id_sucursal);
        }
...
export const obtieneDoctor = async (id_doctor: number) => {
    try {
        const [results]: any = await conexion.query(`
            SELECT d.*, u.nombre, u.apellido_paterno, u.apellido_materno, u.correo, u.telefono
            FROM doctor d
        ...
        if (results.length === 0) return { error: 'No se encuentra el doctor' };

        const [suc]: any = await conexion.query(
            'SELECT id_sucursal FROM doctor_sucursal WHERE id_doctor = ?',
            [id_doctor]
        );
        results[0].sucursales = suc.map((s: any) => s.id_sucursal);
...
export const registraDoctor = async (nuevo: DoctorNuevo) => {
    try {
        ...
        const [correoExiste]: any = await conexion.query(
            'SELECT id_usuario FROM usuario WHERE correo = ? LIMIT 1', [nuevo.correo]
        );
        if (correoExiste.length > 0) return { error: 'Ya existe un usuario con ese correo' };

        const [rfcExiste]: any = await conexion.query(
            'SELECT id_doctor FROM doctor WHERE rfc = ? LIMIT 1', [nuevo.rfc]
        );
        if (rfcExiste.length > 0) return { error: 'Ya existe un doctor con ese RFC' };
...
export const editaDoctor = async (datos: DoctorEditar) => {
    try {
        ...
        if (existe.length === 0) return { error: 'No se encuentra el doctor' };

        await conexion.query(
            `UPDATE usuario SET nombre=?, apellido_paterno=?, apellido_materno=?, correo=?, telefono=? WHERE id_usuario=?`,
