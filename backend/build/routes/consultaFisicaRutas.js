...
router.get('/paciente/:id_paciente', async (req, res) => {
    const resultado = await consultaFisicaServices.obtieneConsultasPorPaciente(Number(req.params.id_paciente));
    res.send(resultado);
});
// http://localhost:3001/api/consultafisica/expediente/1
router.get('/expediente/:id_expediente', async (req, res) => {
...
router.post('/', async (req, res) => {
    try {
        const { id_cita, id_expediente, id_doctor, motivo_consulta, peso_kg, talla_cm, tension_arterial, temperatura_c, frecuencia_cardiaca, notas_examen_fisico, notas_clinicas, tratamiento, indicaciones } = req.body;
...
router.put('/', async (req, res) => {
    try {
        const { id_consulta, id_cita, id_expediente, id_doctor, motivo_consulta, peso_kg, talla_cm, tension_arterial, temperatura_c, frecuencia_cardiaca, notas_examen_fisico, notas_clinicas, tratamiento, indicaciones, firmada } = req.body;
...
router.put('/firmar', async (req, res) => {
    try {
