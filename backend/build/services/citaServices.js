...
export const obtieneCitasPorSucursal = async (id_sucursal) => {
    try {
...
export const registraCita = async (nuevo) => {
    try {
        ...
        if (nuevo.metodo_pago === 'Saldo') {
            const [paciente] = await conexion.query('SELECT saldo_pendiente FROM paciente WHERE id_paciente = ? LIMIT 1', [nuevo.id_paciente]);
            if (paciente[0].saldo_pendiente < montoAnticipo) {
                return { error: 'Saldo insuficiente para cubrir el anticipo' };
            }
            await conexion.query('UPDATE paciente SET saldo_pendiente = saldo_pendiente - ? WHERE id_paciente = ?', [montoAnticipo, nuevo.id_paciente]);
        }
        // Notificacion al paciente
        await notificacionServices.creaNotificacion({
            id_usuario: (await conexion.query('SELECT id_usuario FROM paciente WHERE id_paciente = ?', [nuevo.id_paciente]))[0][0].id_usuario,
        ...
        const [doctorInfo] = await conexion.query(`SELECT u.id_usuario, u.nombre, u.apellido_paterno,
                    c.fecha, c.hora_inicio
            FROM doctor d
            INNER JOIN usuario u ON d.id_usuario = u.id_usuario
            INNER JOIN cita c ON c.id_cita = ?
            WHERE d.id_doctor = c.id_doctor LIMIT 1`, [result.insertId]);
...
export const cancelaCita = async (id_cita, motivo, cancelado_por) => {
    try {
        ...
        // Notificacion al paciente
        const [pacienteCita] = await conexion.query('SELECT p.id_usuario FROM paciente p INNER JOIN cita c ON p.id_paciente = c.id_paciente WHERE c.id_cita = ?', [id_cita]);
        await notificacionServices.creaNotificacion({
        ...
        if (aplicaReembolso) {
            const [anticipo] = await conexion.query("SELECT monto FROM pago WHERE id_cita = ? AND tipo_pago = 'Anticipo' AND estado = 'Completado' LIMIT 1", [id_cita]);
            if (anticipo.length > 0) {
                const [pacienteInfo] = await conexion.query('SELECT id_paciente FROM cita WHERE id_cita = ? LIMIT 1', [id_cita]);
                if (pacienteInfo.length > 0) {
                    await conexion.query('UPDATE paciente SET saldo_pendiente = saldo_pendiente + ? WHERE id_paciente = ?', [anticipo[0].monto, pacienteInfo[0].id_paciente]);
                }
            }
        }
        // Notificación al doctor
        const [doctorCita] = await conexion.query(`SELECT u.id_usuario FROM doctor d
            INNER JOIN usuario u ON d.id_usuario = u.id_usuario
            INNER JOIN cita c ON c.id_doctor = d.id_doctor
            WHERE c.id_cita = ? LIMIT 1`, [id_cita]);
...
export const cancelaCitaDoctor = async (id_cita, motivo, cancelado_por) => {
    try {
        ...
        const [anticipo] = await conexion.query("SELECT monto FROM pago WHERE id_cita = ? AND tipo_pago = 'Anticipo' AND estado = 'Completado' LIMIT 1", [id_cita]);
        // Notificacion al paciente
        const [pacienteCita] = await conexion.query(`SELECT p.id_usuario, u.nombre, u.apellido_paterno,
                c.fecha, c.hora_inicio
            FROM paciente p
