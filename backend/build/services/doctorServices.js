...
export const borrarDoctor = async (id_doctor) => {
    try {
        ...
        const [citasFuturas] = await conexion.query(`SELECT c.id_cita, c.fecha, c.hora_inicio, p.id_usuario AS id_usuario_paciente
             FROM cita c
             INNER JOIN paciente p ON c.id_paciente = p.id_paciente
             WHERE c.id_doctor = ? AND c.estado = 'Programada' AND c.fecha >= CURDATE()`, [id_doctor]);
        for (const cita of citasFuturas) {
            ...
            await conexion.query(`INSERT INTO cancelacion(id_cita, cancelado_por, motivo, aplica_reembolso, porcentaje_reembolso)
                 VALUES(?, ?, 'Doctor eliminado del sistema', 1, 100.00)`, [cita.id_cita, id_usuario]);
