...
export const editaPaciente = async (id_paciente, datos) => {
    try {
        const [results] = await conexion.query('UPDATE paciente SET fecha_nacimiento = ?, sexo = ?, tipo_sangre = ? WHERE id_paciente = ?', [datos.fecha_nacimiento, datos.sexo, datos.tipo_sangre, id_paciente]);
        return results;
    }
    catch (err) {
